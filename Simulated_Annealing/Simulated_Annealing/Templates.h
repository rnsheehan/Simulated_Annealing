		...
		try{
			...
			if(c4){
				...
				for (int i = 0; i < (int)(vec1.size()); i++) {
					Data.push_back( std::make_pair( vec1[i], vec2[i] ) );
				}

				/*std::vector<T>::iterator it1 = vec1.begin(); 
				std::vector<T>::iterator it2 = vec2.begin();*/ 
				/*std::iterator<T> it1 = vec1.begin(); 
				std::iterator<T> it2 = vec2.begin();*/
				...
				for (int i = 0; i < (int)(Data.size()); i++) {
					vec1[i] = Data[i].first; vec2[i] = Data[i].second; 
				}

				//std::vector< std::pair<T,T> >::iterator pit = Data.begin(); 
				/*std::iterator<T> pit = Data.begin(); 
				int i=0; 
